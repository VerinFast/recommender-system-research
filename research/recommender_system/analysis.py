...
def find_most_popular(matrix: NDArray[np.float_], n: int, count_dislikes: bool = const.COUNT_NEGATIVE_REVIEWS) -> list[int]:
...
def find_least_popular(matrix: NDArray[np.float_], n: int, count_dislikes: bool = const.COUNT_NEGATIVE_REVIEWS) -> list[int]:
...
def all_most_popular_recommended(user: ppl.Person, matrix: NDArray[np.float_], top_results: int) -> int:
	"""Determine if the given user used all top_results goods.
	...
	Returns:
			int: Return 1 if the user used all top_results goods, and 0 if they did not
	"""
	...
	return 1 if sum(np.isin(used, most_popular)) == top_results else 0

def num_most_popular_recommended(user: ppl.Person, matrix: NDArray[np.float_], top_results: int) -> int:
	"""Determine how many of the top_results number of most popular goods were used by the given user.
...
def find_optimal_utility(user: ppl.Person) -> float:
	...
	return sum(heapq.nlargest(sum(~np.isnan(user.reviews)), user.utility))
