...
def green_or_red(string, green_logic, red_logic = True, space = False) -> str:
	return ('\033[32m' + (' ' if space == True else '') if green_logic else ('\033[31m' if red_logic else ' ')) + str(string)
...
### What percentage of the most popular goods did the user consume?
...
### How much more likely was a popular suggestion to be recommended? (e.g. how many of top 1, 5, 10 were recommended vs. bottom 1, 5, 10)
...
### What was the utility of the recommendations?
...
# Randomly select goods for the random user to use
...
print(f"A new user who uses the recommender system generates \033[96m{round(new_user.generated_utility - rand_user.generated_utility, 1)}\033[0m (\033[96;4m{round((new_user.generated_utility - rand_user.generated_utility) / const.UTILITY_MEAN, 1)}x\033[0m mean) more utility compared to a user who chooses goods randomly")
